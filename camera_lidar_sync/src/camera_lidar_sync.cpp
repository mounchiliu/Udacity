...
void CameraLidarSync::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    */
    ...
    这种保存多张图像的做法主要有几点：
    1）可以防止由于传输延时导致的时间戳间隔不固定，所以尽可能保持比较多的信息；
    2）可以防止帧率不稳定导致的丢帧，这帧丢了用上一帧；
    3）可以对该容器内的图像进行时间同步，即由于以上两个原因，对图像的时间戳做自校正；
    */ 
...
{
    int index = -1; // 图像队列里与点云相差最小帧的序列号保存
    double time = laser_scan->header.stamp.toSec();
    /*
    作业1：这里并未对图像容器内的时间进行同步自校正，可以先对容器内的时间进行同步
    ...
    */
    ...
    for (auto image_ : images_)
    {
        ...
        //为啥要加个time_diff < 0呢？--- 此时已经找到以雷达为中心 左边帧的相机的ts，这里的判断是右边帧的ts比左边帧的还小
    ...
    */
    ...
    if (point_cloud_lists_.size() > 2) //数量大于2 remove先来的
    {
        point_cloud_lists_.erase(point_cloud_lists_.begin());
    }
    else if (point_cloud_lists_.size() < 2) //数量小于2 push新来的 
    {
        point_cloud_lists_.push_back(*laser_cloud);
        return;
    }
    else // 只处理等于2的，即当两帧相邻点云都有了
    {
        ...
        if(index != -1)
        {
            ...
            // iter为list里第一帧的指针，advance为第一帧指针的便宜到index-1
            ...
            */
            ...
            ndt.setMaximumIterations (500); // 50
            ...
            Eigen::Matrix4f init_guess = Eigen::Matrix4f::Identity ();//这里是没有输入估计

            //最后进行点云配准，变换后的点云保存在output_cloud里，之后打印出配准分数。分数通过计算output_cloud与target cloud对应的最近点欧式距离的平方和得到，得分越小说明diff越小，匹配效果越好。
            ...
            // 比如车静止的时候或者缓慢运行的时候，只需要运行第一次，后面直接用算好的T做变换即可，可极大的减少工作量 所以这里comment了
            // // Transforming unfiltered, input cloud using found transform.
            // pcl::transformPointCloud (second_cloud, *output_cloud_final, ndt.getFinalTransformation ());
    ...
    //对齐后进行校验，通过标定外参进行校验，这里本来想采用cv::solvePnP来进行验证的，这里没有做点云的特征检测和图像的特征检测，所以暂时放弃solvePnP方法，采用直接外参的形式进行比较
